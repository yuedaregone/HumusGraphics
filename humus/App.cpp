...
bool App::init()
{
	...
	if (!m_Map.loadT3d("../Models/PillarRoom/Map.t3d")) return false;
...
{
	// Shaders
	if ((m_Lighting[CLUSTERED]   = renderer->addShader("Lighting.shd", DEFINE_STR(LIGHT_COUNT))) == SHADER_NONE) return false;
	if ((m_Lighting[NO_CLUSTERS] = renderer->addShader("Lighting.shd", DEFINE_STR(LIGHT_COUNT) "#define NO_CLUSTERS\n")) == SHADER_NONE) return false;
	if ((m_Lighting[VISUALIZE]   = renderer->addShader("Lighting.shd", DEFINE_STR(LIGHT_COUNT) "#define SHOW_CLUSTERS\n")) == SHADER_NONE) return false;
	if ((m_PreZ      = renderer->addShader("PreZ.shd")) == SHADER_NONE) return false;
	if ((m_LightBlob = renderer->addShader("LightBlob.shd")) == SHADER_NONE) return false;
	...
	// Textures
	if ((m_Base[0] = renderer->addTexture  ("../Textures/floor_wood_3.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[0] = renderer->addNormalMap("../Textures/floor_wood_3Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;

	if ((m_Base[1] = renderer->addTexture  ("../Textures/floor_wood_4.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[1] = renderer->addNormalMap("../Textures/floor_wood_4Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;

	if ((m_Base[2] = renderer->addTexture  ("../Textures/wallpaper20.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[2] = renderer->addNormalMap("../Textures/wallpaper20Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;
